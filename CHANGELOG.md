...
- Zotero 기존 항목의 컬렉션 키를 읽어 목표 컬렉션 범위에서만 원격 중복을 판정
- 다른 컬렉션에 같은 논문이 있어도 현재 등록 대상 컬렉션에는 신규 메타데이터 항목을 생성
- 같은 문서 안에서 동일하거나 유사한 참고문헌이 반복되면 `source:<번호>`를 가리키는
  `needs_review`로 분류
- 검토 미리보기에 문서 내부 중복과 대상 컬렉션 기존 항목 유사성을 구분해 표시
- 중복 판정 회귀 테스트와 0.2.3 버전 메타데이터 갱신
- 개인 문서 기반 회귀 fixture 보호를 위해 `tests/`를 공개 저장소와 배포 패키지에서 제외
