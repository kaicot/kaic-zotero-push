...
- 입력 파일, 매니페스트, 개인 라이브러리, 기존 컬렉션 키 또는 새 컬렉션 생성
  의도가 모두 같을 때만 승인이 유효합니다.
...
- 승인 후 정확한 이름의 누락 컬렉션 생성 및 생성 key 재사용
...
`--collection`으로 지정한 정확한 이름이 개인 라이브러리에 없으면 미리보기에
**새 컬렉션 생성 예정**으로 표시됩니다. 이 단계에서는 Zotero를 변경하지 않습니다.
사용자가 해당 미리보기를 승인한 뒤 `commit`을 실행할 때만 라이브러리 루트에
컬렉션을 한 번 생성하고, 반환된 key를 모든 신규 항목에 사용합니다. 같은 이름이
여러 개이면 임의로 선택하거나 생성하지 않고 중단합니다.
...
- 기존 항목을 새 컬렉션에 추가하거나 이동하는 기능 미지원
- 하위 컬렉션 생성 및 동명 컬렉션 자동 선택 미지원
